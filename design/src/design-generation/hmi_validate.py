...
# Мир холста и его сетка — из спецификации импорта, §0
# Размер нарисованной сетки в редакторе. Мира это не ограничивает:
# клампа позиций нет, камера не ограничена, холст Konva аллоцируется
# по видимой области. Число оставлено только затем, чтобы показывать
# в отчёте, насколько лист больше этой сетки
CANVAS_SIZE = 5000.0
GRID = 20.0
MIN_FONT_SIZE = 12.0
...
def _check_canvas(elements: List[Dict[str, Any]], by_key: Dict[Any, Dict[str, Any]],
    ...
    # Замечанием это больше не считается. Раньше проверка шла от «мира
    # 5000x5000» из спецификации, и лист A0 стоял красным всегда.
    # На поверку 5000 в редакторе — это
    # протяжённость нарисованной сетки, координаты ничем не ограничены,
    # а нижнюю границу зума они опускают до 0.05, чтобы открывался
    # и A0. Размер листа остаётся в отчёте числом — он важен сам по себе
    if far_x > CANVAS_SIZE or far_y > CANVAS_SIZE:
        stats["за_холстом"] = 1
...
def format_report(problems: List[str], stats: Dict[str, int], limit: int = 10) -> str:
    ...
    if stats.get("за_холстом"):
        lines.append(f"схема {stats['холст_ширина']}x{stats['холст_высота']} "
                     f"больше нарисованной сетки {CANVAS_SIZE:.0f}x"
                     f"{CANVAS_SIZE:.0f} — это не предел, размер листа "
                     f"задаётся CONTUR_HMI_SYMBOL_CELLS")
    return "\n".join(lines)
