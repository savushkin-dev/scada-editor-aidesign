...
#   - координаты числами — это пиксели холста, а проценты строкой считаются
#     от размера листа (§7a), а без элемента meta — от 5000 по обеим осям.
#     Лист не квадратный: в процентах A0 растянулся бы с 1.41:1 до 1:1.
#     Поэтому координаты выгружаются числами;
...
# Сетка холста редактора (спецификация импорта, §0). Шаг зашит в редакторе
# и не настраивается; минимальный осмысленный объект — две клетки.
# Размер листа сцены задаётся отдельно, блоком canvas элемента meta (§7a):
# он ничего не ограничивает и служит рамкой, за которую видно, где кончается
# бумага. Предела координатам у редактора нет
GRID = 20.0
MIN_OBJECT_SIZE = 2 * GRID


def _ceil_grid(value: float) -> float:
    """Вверх до узла сетки: размеры листа кратны 20 (§7a)."""
...
class HMIExporter:
    ...
    def _sheet_scale(self) -> float:
        ...
        Холст размечен сеткой 20, символ устройства на листе около
        31 пункта. Под кружок в две клетки лист растягивался в 1.28 раза,
        под фигуру библиотеки в десять клеток — примерно в 6.4.

        Множитель один на весь лист, и ничем сверху не ограничен: размер
        сцены — это рамка, а не предел (§7a), координаты за ней работают
        как обычно. Ужимать лист под какое-то число нельзя — ужмётся
        и готовая фигура, а приехать она должна такой, какой нарисована.
        """
    ...
    def _meta(self, elements: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Размер листа: по нему редактор рисует рамку сцены и считает
        # координаты, заданные процентом. Меряется по содержимому — чертёж
        # сдвинут в начало координат, и его дальний край и есть край листа.
        # У отрезка x, y — середина, поэтому край берётся по концам,
        # а не по габариту
        far_x, far_y = 0.0, 0.0
        for element in elements:
        ...
        # Вверх до узла сетки: размер листа кратен 20. Обычно он таким
        # и выходит — крайним оказывается рамка чертежа или надпись,
        # а они на сетку сажаются. Но у диагоналей и звеньев кривых
        # координаты точные, и стоит такой линии оказаться дальше всех,
        # как край листа перестаёт быть кратным
        extent = (_ceil_grid(far_x), _ceil_grid(far_y))
