...
def main() -> int:
        ...
        if not Path(args.svg).exists():
            ...
            if ok:
                ...
                if step("экспорт для редактора", lambda: export_current_visualization_hmi(
                    ...
                    # но не замечание: предела холста у редактора нет,
                    # 5000 — протяжённость нарисованной сетки
                    # (PROJECT.md, §6.2)
                    if drawing_stats.get("за_холстом"):
                        print(f"         лист {drawing_stats['холст_ширина']}x"
                              f"{drawing_stats['холст_высота']} — больше "
                              f"нарисованной сетки 5000x5000, размер задаётся "
                              f"CONTUR_HMI_SYMBOL_CELLS")
    ...
    # Сверка с эталоном. Числа конвейера раньше жили только в STATUS.md,
    # то есть сверялись глазами и по памяти — так и приезжали тихие регрессии
    print("\nСверка с эталоном:")
